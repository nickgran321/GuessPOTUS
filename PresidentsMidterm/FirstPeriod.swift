...
struct FirstPeriod: View {
    ...
    @State var presidents = ["George Washington", "John Adams", "Thomas Jefferson", "James Madison", "James Monroe", "John Quincy Adams", "Andrew Jackson", "Martin Van Buren", "William Henry Harrison", "John Tyler", "James K. Polk"].shuffled()
    
    @State var correctAnswer = Int.random(in: 0...2)
    @State var score = UserDefaults.standard.integer(forKey: "ScoreFirstPeriod")
    @State var rotation = 1
    @State var selectedCorrectPresident = true
    @State var showListScreen = false
    @State var userLevel = UserDefaults.standard.integer(forKey: "LevelFirstPeriod")
    ...
    func presidentTapped(_ number: Int) {
        if number == correctAnswer {
            ...
            score += 30
            UserDefaults.standard.set(self.score, forKey: "ScoreFirstPeriod")
