...
struct SecondPeriod: View {
    ...
    @State var presidents = ["Zachary Taylor", "Millard Fillmore", "Franklin Pierce", "James Buchanan", "Abraham Lincoln", "Andrew Johnson", "Ulysses S. Grant", "Rutherford B. Hayes", "James Garfield", "Chester A. Arthur", "Grover Cleveland"].shuffled()
    
    @State var correctAnswer = Int.random(in: 0...2)
    @State var score = UserDefaults.standard.integer(forKey: "ScoreSecondPeriod")
    @State var rotation = 1
    @State var selectedCorrectPresident = true
    @State var showListScreen = false
    @State var userLevel = UserDefaults.standard.integer(forKey: "LevelSecondPeriod")
    ...
    func presidentTapped(_ number: Int) {
        if number == correctAnswer {
            ...
            score += 30
            UserDefaults.standard.set(self.score, forKey: "ScoreSecondPeriod")
