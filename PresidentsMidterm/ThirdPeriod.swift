...
struct ThirdPeriod: View {
    ...
    @State var presidents = ["Benjamin Harrison", "Grover Cleveland", "William McKinley", "Theodore Roosevelt", "William Howard Taft", "Woodrow Wilson", "Warren G. Harding", "Calvin Coolidge", "Herbert Hoover", "Franklin D. Roosevelt", "Harry S. Truman"].shuffled()
    
    @State var correctAnswer = Int.random(in: 0...2)
    @State var score = UserDefaults.standard.integer(forKey: "ScoreThirdPeriod")
    @State var rotation = 1
    @State var selectedCorrectPresident = true
    @State var showListScreen = false
    @State var userLevel = UserDefaults.standard.integer(forKey: "LevelThirdPeriod")
    ...
    func presidentTapped(_ number: Int) {
        if number == correctAnswer {
            ...
            score += 30
            UserDefaults.standard.set(self.score, forKey: "ScoreThirdPeriod")
